...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        mbti_tokens = [f"<MBTI_{t}>" for t in mbti_types]
        added_tokens = self.tokenizer.add_special_tokens({'additional_special_tokens': mbti_tokens})
        if self.device_mesh.get_rank() == 0:
            print(f"Added {added_tokens} MBTI tokens to tokenizer.")
